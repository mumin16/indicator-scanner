...
REPORT = { buys = {}, sells = {}, profits = {}, opts = {}, gains = {}, loses = {} }
...
function WEIGHTEDPRICE()
  for i = 1, #CLOSE, 1 do
    WEIGHTED[i] = (HIGH[i] + LOW[i] + CLOSE[i] + CLOSE[i]) / 4
  end
  return WEIGHTED
end
...
function HMA(source, period)
  ...
  function LinearWeightedMA(position, period, price)
    local result = 0.0
    local sum = 0.0;
    local wsum = 0;
    if (position >= period and period > 0) then
      for i = period, 1, -1 do
        wsum = wsum + i;
        sum = sum + price[position - i + 1] * (period - i + 1);
      end
      result = sum / wsum;
    end
    return result
  end
  local p = math.floor(math.sqrt(period));
  ...
  for i = 1, #out, 1 do
    if i ~= 1 and out[i] > out[i - 1] then colorbuffer[i] = CLR_GREEN else colorbuffer[i] = CLR_RED end
  end
...
function EMA(source, period)
  ...
  local out = {}
  for i = 1, #source, 1 do
    if i == 1 then out[i] = source[i] else out[i] = (source[i] - out[i - 1]) * multiplier + out[i - 1] end
  end
  return out
end
...
function STDDEV(source, period, matype)
  ...
  elseif matype == 1 then
    sma = EMA(source, period)
  elseif matype == 2 then
    sma = SMMA(source, period)
  elseif matype == 3 then
    sma = LWMA(source, period)
  end
  local out = {}
  for i = 1, #sma, 1 do
    local sum = 0
...
function BollingerBands(source, period, deviation)
  ...
  for i = 1, #sma, 1 do
    ...
    upper[i] = sma[i] + math.sqrt(sum / period) * deviation
    lower[i] = sma[i] - math.sqrt(sum / period) * deviation
  end
...
function DEMARKER(period)
  ...
  for i = 1, #HIGH - period + 1, 1 do
    out[i] = demaxsma[i] / (demaxsma[i] + deminsma[i])
  end
  return out
end
...
function ATR(period)
  ...
  for i = 1, #CLOSE, 1 do
    if i == 1 then
      ...
      tr[i] = math.max((HIGH[i] - LOW[i]), math.abs((CLOSE[i - 1] - HIGH[i])), math.abs((CLOSE[i - 1] - LOW[i])))
    end
  end
...
function W_AD()
  function IsEqualDoubles(d1, d2, epsilon)
    if (epsilon < 0.0) then epsilon = -epsilon end
    if (epsilon > 0.1) then epsilon = 0.00001 end
    --
    local diff = d1 - d2;
    if (diff > epsilon or diff < -epsilon) then return (false); end
    --
    return (true);
  end
  local ExtWADBuffer = {}
  ...
  for i = pos, #CLOSE, 1 do
    ...
    local lo = LOW[i];
    local cl = CLOSE[i];
    local prev_cl = CLOSE[i - 1];
    -- calculate TRH and TRL
    local trh = math.max(hi, prev_cl);
    local trl = math.min(lo, prev_cl);
    -- calculate WA/D
    if (IsEqualDoubles(cl, prev_cl, 0.00001)) then
...
function STOCHASTICSIGNAL(Kperiod, Dperiod, slowing, matype)
  local sma = {}
  if matype == 0 then
    sma = SMA(STOCHASTIC(Kperiod, slowing), Dperiod)
  elseif matype == 1 then
    sma = EMA(STOCHASTIC(Kperiod, slowing), Dperiod)
  elseif matype == 2 then
    sma = SMMA(STOCHASTIC(Kperiod, slowing), Dperiod)
  elseif matype == 3 then
    sma = LWMA(STOCHASTIC(Kperiod, slowing), Dperiod)
  end
  return sma
end
...
function ASI(InpT)
  ...
  for i = pos, #CLOSE, 1 do
    ...
    --- calculate SI value
    if R == 0.0 or ExtTpoints == 0.0 then
      ExtSIBuffer[i] = 0.0;
    else
      ExtSIBuffer[i] = 50 * (dClose - dPrevClose + 0.5 * (dClose - OPEN[i]) +
...
function MI(ExtPeriodEMA, ExtSecondPeriodEMA, ExtSumPeriod)
  function ExponentialMA(position, period, prev_value, price)
    local result = 0.0;
    if (period > 0) then
...
function CHV(ExtCHVPeriod, ExtSmoothPeriod, matype)
  ...
  if matype == 0 then
    ExtSHLBuffer = SMA(ExtHLBuffer, ExtSmoothPeriod)
  elseif matype == 1 then
    ExtSHLBuffer = EMA(ExtHLBuffer, ExtSmoothPeriod)
  end
  --- calculate CHV buffer
  local ExtCHVBuffer = {}
  local limit = 0
  if ExtCHVPeriod >= ExtSmoothPeriod then limit = ExtCHVPeriod else limit = ExtSmoothPeriod end
  for i = 1 + ExtCHVPeriod, #ExtSHLBuffer, 1 do
    ExtCHVBuffer[i - ExtCHVPeriod] = 100.0 * (ExtSHLBuffer[i] - ExtSHLBuffer[i - ExtCHVPeriod]) /
        ExtSHLBuffer[i - ExtCHVPeriod];
  end
  return ExtCHVBuffer
end
...
function AMA(price, amaperiod, ExtFastPeriodEMA, ExtSlowPeriodEMA)
  ...
  function CalculateER(nPosition, PriceData, ExtPeriodAMA)
    local dSignal = math.abs(PriceData[nPosition] - PriceData[nPosition - ExtPeriodAMA]);
    local dNoise = 0.0;
    for delta = 0, ExtPeriodAMA - 1, 1 do
      dNoise = dNoise + math.abs(PriceData[nPosition - delta] - PriceData[nPosition - delta - 1]);
    end
...
function AROON(period)
  function HighestSince(StartPos, Depth)
    local res = HIGH[StartPos - Depth + 1]
    local ago = Depth
    for i = 1, Depth, 1 do
      if HIGH[StartPos - Depth + i] > res then
        res = HIGH[StartPos - Depth + i]
        ago = Depth - i
      end
    end
    return (ago);
  end
  function LowestSince(StartPos, Depth)
    local res = LOW[StartPos - Depth + 1]
    local ago = Depth
    for i = 1, Depth, 1 do
      if LOW[StartPos - Depth + i] < res then
        res = LOW[StartPos - Depth + i]
        ago = Depth - i
      end
    end
    return (ago);
  ...
  for i = period, #CLOSE, 1 do
    ...
    up[i - period + 1] = ((period - hsince) / period) * 100
    local lsince = LowestSince(i, period);
    down[i - period + 1] = ((period - lsince) / period) * 100    
  end
  return up,down
end
...
function VIDYA(source, InpPeriodCMO, InpPeriodEMA)
  ...
  function CalculateCMO(Position, PeriodCMO, price)
    local resCMO = 0.0;
    local UpSum = 0.0
    local DownSum = 0.0;
...
function VROC(ExtPeriodVROC)
  ...
  for i = ExtPeriodVROC, #CLOSE, 1 do
    ...
    if (PrevVolume ~=0) then
      ...
      ExtVROCBuffer[i - ExtPeriodVROC + 1] = ExtVROCBuffer[i - ExtPeriodVROC];
    end
  end
...
function CEXITLONG(period, ATRPeriod, MultipleATR)
  ...
  local atr = ATR(ATRPeriod)
  local diff = #hhv - #atr
  local out = {}
  for i = 1, #atr, 1 do
    out[i] = hhv[i + diff] - atr[i] * MultipleATR
  end
  return out
end
...
function CEXITSHORT(period, ATRPeriod, MultipleATR)
  ...
  local out = {}
  for i = 1, #atr, 1 do
    out[i] = llv[i + diff] + atr[i] * MultipleATR
  end
  return out
end
...
function KELTNERUP(price, period, multiplier)
  ...
  local a = #ema - #atr
  for i = 1, #atr, 1 do
    out[i] = ema[i + a] + atr[i] * multiplier
  end
  return out
end
...
--Coppock Curve = wma(roc(source, longRoCLength) + roc(source, shortRoCLength), wmaLength)
function COPP(r1period, r2period, wperiod)
  local roc1 = ROC(CLOSE, r1period)
  local roc2 = ROC(CLOSE, r2period)
  local diff = #roc2 - #roc1
  local out = {}
  for i = 1, #roc1, 1 do
    out[i] = roc1[i] + roc2[i + diff]
  end
...
function iSum(period, source, start)
  local sum = 0
  for j = 0, period - 1, 1 do
    sum = sum + source[start - j]
  end
  return sum
end
...
function UltimateOsc(InpFastPeriod, InpMiddlePeriod, InpSlowPeriod, InpFastK, InpMiddleK, InpSlowK)
  ...
  for i = 2, InpSlowPeriod + 1, 1 do
    BP[i] = CLOSE[i] - math.min(LOW[i], CLOSE[i - 1])
    TR[i] = math.max(HIGH[i], CLOSE[i - 1]) - math.min(LOW[i], CLOSE[i - 1])
  end

  local UO = {}
  for i = InpSlowPeriod, #CLOSE, 1 do
    BP[i] = CLOSE[i] - math.min(LOW[i], CLOSE[i - 1])
    TR[i] = math.max(HIGH[i], CLOSE[i - 1]) - math.min(LOW[i], CLOSE[i - 1])
    local Average7 = iSum(InpFastPeriod, BP, i) / iSum(InpFastPeriod, TR, i)
    local Average14 = iSum(InpMiddlePeriod, BP, i) / iSum(InpMiddlePeriod, TR, i)
    local Average28 = iSum(InpSlowPeriod, BP, i) / iSum(InpSlowPeriod, TR, i)
    UO[i-InpSlowPeriod+1] = 100 * ((InpFastK * Average7) + (InpMiddleK * Average14) + Average28) / (InpFastK + InpMiddleK + InpSlowK)
  end
...
for i = start, #CLOSE, 1 do
        ...
        local t = math.max(CLOSE[i], OPEN[i]);
        Up1[i] = math.max(t, Up1[i - 1] - (Up1[i - 1] - CLOSE[i]) * alpha);
        if Up1[i] == 0 then Up1[i] = CLOSE[i] end

        t = math.max(CLOSE[i] * CLOSE[i], OPEN[i] * OPEN[i]);
        Up2[i] = math.max(t, Up2[i - 1] - (Up2[i - 1] - CLOSE[i] * CLOSE[i]) * alpha);
        if Up2[i] == 0 then Up2[i] = CLOSE[i] * CLOSE[i] end

        t = math.min(CLOSE[i], OPEN[i]);
        Dn1[i] = math.min(t, Dn1[i - 1] + (CLOSE[i] - Dn1[i - 1]) * alpha);
        if Dn1[i] == 0 then Dn1[i] = CLOSE[i] end

        t = math.min(CLOSE[i] * CLOSE[i], OPEN[i] * OPEN[i]);
        Dn2[i] = math.min(t, Dn2[i - 1] + (CLOSE[i] * CLOSE[i] - Dn2[i - 1]) * alpha);
        if Dn2[i] == 0 then Dn2[i] = CLOSE[i] * CLOSE[i] end

        Bull[i] = math.sqrt(Dn2[i] - Dn1[i] * Dn1[i]);
        Bear[i] = math.sqrt(Up2[i] - Up1[i] * Up1[i]);
        Signal[i] = math.max(Bull[i], Bear[i]);
...
function iHighest(array, depth, startPos)
  ...
  if (startPos - depth < 0) then depth = startPos; end
  local max = array[startPos];
  -- start searching
  for i = startPos, startPos - depth + 2, -1 do
    if (array[i] > max) then
      index = i;
      max = array[i];
    end
...
function iLowest(array, depth, startPos)
  local index = startPos;
  -- start index validation
  if (startPos < 0) then
...
function ZIGZAG(ExtDepth, ExtDeviation, ExtBackstep)
  ...
  for shift = limit, rates_total - 1, 1 do
    ...
    if (val == lastlow) then
      ...
      if ((LOW[shift] - val) > deviation) then
        ...
        for back = 1, ExtBackstep, 1 do
          res = LowMapBuffer[shift - back];
          if ((res ~= 0) and (res > val)) then LowMapBuffer[shift - back] = 0.0; end
        end
      end
    end
    if (LOW[shift] == val) then LowMapBuffer[shift] = val; else LowMapBuffer[shift] = 0.0; end
    --- H
    ...
    if (HIGH[shift] == val) then HighMapBuffer[shift] = val; else HighMapBuffer[shift] = 0.0; end
  end



  -- last preparation
  if (whatlookfor == 0) then -- uncertain quantity
    lastlow = 0;
    lasthigh = 0;
  else
    lastlow = curlow;
  ...
  for shift = limit, rates_total - 1, 1 do
    ...
    if (whatlookfor == Pike) then -- search for peak
    ...
    if (whatlookfor == Sill) then -- search for lawn
...
function SUPERTREND(Periode, Multiplier)
  local Atr = ATR(Periode);
  ...
  Up[Periode - 1] = 0.0
  Down[Periode - 1] = 0.0
  Middle[Periode - 1] = 0.0
  trend[Periode - 1] = 0.0

  local colorbuffer = {}

  for i = Periode, rates_total, 1 do
    Middle[i] = (HIGH[i] + LOW[i]) / 2;
    Up[i]     = Middle[i] + (Multiplier * Atr[i - Periode + 1]);
    Down[i]   = Middle[i] - (Multiplier * Atr[i - Periode + 1]);

    local changeOfTrend
    if (CLOSE[i] > Up[i - 1]) then
      trend[i] = 1;
      if (trend[i - 1] == -1) then changeOfTrend = 1; end
    elseif (CLOSE[i] < Down[i - 1]) then
      trend[i] = -1;
      if (trend[i - 1] == 1) then changeOfTrend = 1; end
    ...
    local flag
    if (trend[i] < 0 and trend[i - 1] > 0) then
      flag = 1;
    else
      flag = 0;
    end
    ...
    if (trend[i] > 0 and Down[i] < Down[i - 1]) then Down[i] = Down[i - 1]; end

    if (trend[i] < 0 and Up[i] > Up[i - 1]) then Up[i] = Up[i - 1]; end

    if (flag == 1) then Up[i] = Middle[i] + (Multiplier * Atr[i - Periode + 1]); end

    if (flagh == 1) then Down[i] = Middle[i] - (Multiplier * Atr[i - Periode + 1]); end

    -- Draw the indicator
    if (trend[i] == 1) then
      SuperTrend[i-Periode+1] = Down[i];
      if (changeOfTrend == 1) then
        SuperTrend[i - 1-Periode+1] = SuperTrend[i - 2-Periode+1];
        changeOfTrend = 0;
      end
    elseif (trend[i] == -1) then
      SuperTrend[i-Periode+1] = Up[i];
      if (changeOfTrend == 1) then
        SuperTrend[i - 1-Periode+1] = SuperTrend[i - 2-Periode+1];
        changeOfTrend = 0;
      end
    ...
    if (CLOSE[i] > SuperTrend[i-Periode+1]) then
      colorbuffer[i - Periode + 1] = CLR_GREEN
    else
      colorbuffer[i - Periode + 1] =
          CLR_RED
    end
  end
  ...
  return SuperTrend, colorbuffer
end


local workMama={}
local function iMama( price_,  fastLimit,  slowLimit,    r,  bars)
...
local rad2degree=180.0 / 3.14159265358979323846

local function calcComp(_i, _ind) 
  if _i > 5 then 
    return (0.0962 * workMama[_i][_ind] + 0.5769 * workMama[_i - 2][_ind] - 0.5769 * workMama[_i - 4][_ind] - 0.0962 * workMama[_i - 6][_ind]) * (0.075 * workMama[_i - 1][_period] + 0.54) 
    else return workMama[_i][_ind]
    end
end

  workMama[r]={} 

  workMama[r][_price] = price_;
  if r > 3 then workMama[r][_smooth]=(4.0 * workMama[r][_price] + 3.0 * workMama[r - 1][_price] + 2.0 * workMama[r - 2][_price] + workMama[r - 3][_price]) / 10.0 else workMama[r][_smooth]=price_ end
  workMama[r][_detrender] = calcComp(r, _smooth);
  workMama[r][_Q1] = calcComp(r, _detrender);
   if (r > 2) then workMama[r][_I1] =workMama[r - 3][_detrender] else workMama[r][_I1] =workMama[r][_detrender]; end
  workMama[r][_JI] = calcComp(r, _I1);
  workMama[r][_JQ] = calcComp(r, _Q1);



  if (r == 0) then workMama[r][_I2] = workMama[r][_I1] else workMama[r][_I2] = 0.2 * (workMama[r][_I1] - workMama[r][_JQ]) + 0.8 * workMama[r - 1][_I2]; end
   if (r == 0) then workMama[r][_Q2] =workMama[r][_Q1] else workMama[r][_Q2] =0.2 * (workMama[r][_Q1] + workMama[r][_JI]) + 0.8 * workMama[r - 1][_Q2]; end
   if (r == 0) then workMama[r][_Re] =workMama[r][_I2] else workMama[r][_Re] =0.2 * (workMama[r][_I2] * workMama[r - 1][_I2] + workMama[r][_Q2] * workMama[r - 1][_Q2]) + 0.8 * workMama[r - 1][_Re]; end
   if (r == 0) then workMama[r][_Im] =workMama[r][_I2] else workMama[r][_Im] =0.2 * (workMama[r][_I2] * workMama[r - 1][_Q2] - workMama[r][_Q2] * workMama[r - 1][_I2]) + 0.8 * workMama[r - 1][_Im]; end

  if (workMama[r][_Re] ~= 0 and workMama[r][_Im] ~= 0) then
    workMama[r][_period] = 360.0 / (math.atan(workMama[r][_Im] / workMama[r][_Re]) * rad2degree); end
  if (r == 0) then workMama[r][_period] = workMama[r][_period] else workMama[r][_period] = math.min(workMama[r][_period], 1.50 * workMama[r - 1][_period]); end
  if (r == 0) then workMama[r][_period] = workMama[r][_period] else workMama[r][_period] = math.max(workMama[r][_period], 0.67 * workMama[r - 1][_period]); end
  workMama[r][_period] = math.min(math.max(workMama[r][_period], 6.0), 50.0);
  if (r == 0) then workMama[r][_period] = workMama[r][_period] else workMama[r][_period] = 0.2 * workMama[r][_period] + 0.8 * workMama[r - 1][_period]; end


local DeltaPhase
local Alpha

  if (workMama[r][_I1] ~= 0) then
    workMama[r][_phase] = math.atan(workMama[r][_Q1] / workMama[r][_I1]) * rad2degree; end
   if (r == 0) then DeltaPhase =0 else  DeltaPhase =math.max(workMama[r - 1][_phase] - workMama[r][_phase], 1.0); end
  if (DeltaPhase ~= 0) then Alpha = math.max(math.min(fastLimit / DeltaPhase, fastLimit), slowLimit) else  Alpha = 1.0; end
  workMama[r][_sa] = Alpha;



   if (r == 0) then workMama[r][_mama] =price_ else workMama[r][_mama] =workMama[r][_sa] * workMama[r][_price] + (1.0 - workMama[r][_sa]) * workMama[r - 1][_mama]; end
   if (r == 0) then workMama[r][_fama] =price_ else workMama[r][_fama] =0.5 * workMama[r][_sa] * workMama[r][_mama] + (1.0 - 0.5 * workMama[r][_sa]) * workMama[r - 1][_fama]; end
 
 
  return workMama[r][_mama] , workMama[r][_fama];
...
function MamaFamaOsc( src, inpFastLimit,  inpSlowLimit)
  ...
  for  i = 0, #src-1, 1 do
    ...
    valm[i+1], valf[i+1] = iMama(src[i+1], inpFastLimit, inpSlowLimit,  i, 0);
...
function TRADE(buy, sell, ...)
  ...
  for i = 1, #CLOSE, 1 do
    if buy[i] == 1 and lastpos == 0 then
      lastpos = 1
      alfiyat = CLOSE[lendiffbuy + i]
      if satfiyat == 0 then ilkyatirim = alfiyat end --print("al",CLOSE[lendiffbuy+i])
      buypos[b] = i
      b = b + 1
    end
    if sell[i] == 1 and lastpos == 1 then
      lastpos = 0
      satfiyat = CLOSE[lendiffsell + i]
      toplamkz = toplamkz + satfiyat - alfiyat --print(toplamkz)
      sellpos[s] = i
      s = s + 1

      if (satfiyat - alfiyat >= 0) then
        gains = gains + 1
      else
        loses = loses + 1
      end
  ...
  --kapaportfoy
  if lastpos == 1 then
    kapafiyat = CLOSE[#CLOSE]                 --print("kapa",CLOSE[#CLOSE]) print("K/Z",kapafiyat-alfiyat)
    toplamkz = toplamkz + kapafiyat - alfiyat --print(toplamkz)
    if (kapafiyat - alfiyat >= 0) then
      gains = gains + 1
    else
      loses = loses + 1
    end
  ...
  if toplamkz == 0 then --hic pozisyon acilamadi
    table.insert(REPORT["profits"], 0)
  else
    table.insert(REPORT["profits"], toplamkz / ilkyatirim * 100)
  end
  table.insert(REPORT["buys"], buypos)
  table.insert(REPORT["sells"], sellpos)
  local arguments = { ... }
  local optss = {};
  for i = 1, #arguments do
    table.insert(optss, arguments[i])
  end
  table.insert(REPORT["opts"], optss)
  table.insert(REPORT["gains"], gains)
  table.insert(REPORT["loses"], loses)
end
